...
// Verifies base-load framing:
//  - /base/:id showing empty space (content far from origin) → auto-fit frames it.
//  - /base/:id whose camera already shows content → auto-fit leaves it alone.
//  - / (local base) → auto-fit never fires.
...
const mockGraphql = async (page, components) => {
    const mocks = {
        ...
        // The query returns the base row too; null = no server config, which
        // is what a board base without the geo columns looks like.
        getComponentsForBase: { base: null, components },
    }
...
test.describe('Fit-to-content on base load', () => {
    test('frames far-away content when opening /base/:id', async ({ page }) => {
    ...
    test('does NOT fire on / (local base)', async ({ page }) => {
        await mockGraphql(page, NEAR_COMPONENTS)
        await page.goto('/')
        await page.waitForSelector('#main-two-root svg')
        await page.waitForTimeout(6000)
    ...
    test('"Go to content" button is hidden on / (local base)', async ({ page }) => {
        await mockGraphql(page, NEAR_COMPONENTS)
        await page.goto('/')
        await page.waitForSelector('#main-two-root svg')
        // Draw a shape so the base has content; the button must still stay
        // hidden because this is the local home base, not /base/:id.
